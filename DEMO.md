...
> **The images below are not committed to the repo.** Generate them first:
>
> ```bash
> python demo.py            # writes everything referenced here into demo/
...
and the center should empty. **It doesn't.** Non-antipodal inner chords pass
near the center in quantity (C(n,4) crossings), so even-n centers still
partially fill; the diameters alone give a sparse star, not a filled region.
The hypothesis is upgraded, not destroyed: antipodality is **necessary but not
sufficient**, and the "filled center" is two superposed mechanisms.
...
![K_5 and K_6, the two balanced graphs](demo/05_balance_points.png)
...
equilibrium" has a literal combinatorial referent.

![Pentagon vs pentagram, 5 edges each](demo/06_pentagon_pentagram.png)
...
balance point; analogous taxonomies for other layouts (sphere, concentric
circles); and whether the failure signatures seen here — momentum errors,
taxonomy decay, resolution hunger — recur across sessions.
...
![Random palette morph](demo/07_random_palette.gif)
...
| 5 | `python kn_explorer.py --show 5` / `--show 6` |
| 6 | `python kn_explorer.py --show 5 --mode adjonly` / `--mode inneronly` |
| 7 | `python kn_explorer.py --animate 4 8 --smooth --random-colors` |
